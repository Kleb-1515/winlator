...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            // TRANSLUCENT A/B test for HWC composition, while avoiding
            // redundant transparency/Z mutations on every frame.
            const bool enteringVulkan =
                setSurfacePresentationMode(window, SurfacePresentationMode::Vulkan);

            if (enteringVulkan) {
                printf("[DisplayX][HWC-TEST] entering Vulkan mode window=%d: applying TRANSLUCENT + Z=1 once",
                       window->id);
