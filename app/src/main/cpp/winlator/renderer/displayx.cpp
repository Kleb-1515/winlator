...
void DisplayX::onCommitCallback(void *context, ASurfaceTransactionStats *stats) {
    // Commit callback runs on the compositor thread — do NOT report work duration
    // here. Wall-clock elapsed between commits equals the frame interval, not the
    // actual CPU work time, so it would mislead the power scheduler. Work duration
    // is reported in presentThreadLoop after each ASurfaceTransaction_apply call.
    (void)context;
    (void)stats;
}
...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            if (pfnASurfaceTransactionSetOnCommit)
...
void DisplayX::createWindowControl(Window *window) {
    ...
    std::lock_guard<std::mutex> lock(windowTransactionMutex);
...
void DisplayX::mapWindow(Window *window) {
    ...
    // Bug fix #7: this used to call SetVisibility(SHOW) unconditionally —
    // even for the window we just marked `enabled = false` two lines above,
    // and even for descendants of a blacklisted window (disableAllDescendants()
    // on the Java side flips WindowAttributes.isEnabled() for those, but this
    // function never checked it before showing the layer). Every such window
    // still became a real, visible SurfaceFlinger layer every frame. Re-check
    // the same "viewable" attribute EGLRenderer already trusts in
    // collectRenderableWindows() before showing the layer at all. This has to
    // happen here (map time), not in createWindowControl (create time):
    // disableAllDescendants() on the Java side only runs during onMapWindow,
    // before nativeMapWindow — it hasn't happened yet when the window is
    // first created, so checking earlier would miss it.
    bool viewable = eventEnv && eventEnv->CallBooleanMethod(window->attributes, cache->windowAttributesIsEnabled);
    if (!viewable) return;
...
void DisplayX::flushWindowTransaction() {
    std::lock_guard<std::mutex> lock(windowTransactionMutex);
    if (!windowTransactionDirty) return;

    pfnASurfaceTransactionApply(windowTransaction);
    windowTransactionDirty = false;
}
...
void DisplayX::restoreControlState() {
    ...
    // Bug fix #6: this used to call ASurfaceTransaction_apply() once per window
    // in the loop below — on resume with N windows that's N separate
    // SurfaceFlinger commits back-to-back, each one a full composition pass.
    // All the per-window state (reparent/visibility/geometry/buffer) is
    // independent and can be folded into the same transaction and committed
    // once after the loop.
    {
        std::lock_guard<std::mutex> lock(windowTransactionMutex);
        ...
        for (const auto& entry : windowTree) {
            ...
            // Same check as mapWindow (bug fix #7): `mapped` alone isn't
            // enough — a mapped window can still be non-viewable (WM_CLASS
            // blacklist / disableAllDescendants()). Without this, every
            // resume would silently re-show windows the map-time filter had
            // correctly hidden.
            bool viewable = window->mapped && eventEnv && eventEnv->CallBooleanMethod(window->attributes, cache->windowAttributesIsEnabled);
            pfnASurfaceTransactionSetVisibility(windowTransaction, window->control, viewable ? ASURFACE_TRANSACTION_VISIBILITY_SHOW : ASURFACE_TRANSACTION_VISIBILITY_HIDE);
