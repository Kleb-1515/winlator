...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            // Keep GPU_SAMPLED_IMAGE on the AHardwareBuffer as a fallback capability:
            // it does not force client composition, but prevents a black layer if HWC
            // rejects a frame and SurfaceFlinger must sample the buffer with RenderEngine.
            //
