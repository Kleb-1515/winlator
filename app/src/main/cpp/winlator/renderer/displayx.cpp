...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            // CPU-drawn / directContent buffer (normal X11 path).
            //
            // These are individual Wine windows (dialogs, menus, tooltips, window
            // decorations, direct-content surfaces) that rely on per-pixel alpha
            // (rounded corners, shadows, translucent popups). Mark them TRANSLUCENT
            // so SurfaceFlinger composites them correctly. Only isDisplayX (Vulkan
            // full-screen) layers are OPAQUE. Applied once on mode entry — no need
            // to re-send on every frame.
            const bool enteringCpu =
                setSurfacePresentationMode(window, SurfacePresentationMode::Cpu);

            if (enteringCpu) {
                printf("[DisplayX] entering CPU/directContent mode window=%d: applying TRANSLUCENT once",
                       window->id);
