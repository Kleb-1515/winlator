...
// HWC A/B test: keep SurfaceControl presentation properties stable during
// steady-state Vulkan presents. Re-sending OPAQUE/Z on every frame creates
// unnecessary layer-state churn and forces SurfaceFlinger/HWC to re-evaluate
// properties that did not actually change.
enum class SurfacePresentationMode : uint8_t {
    Unknown,
    Cpu,
    Vulkan,
};

static std::mutex gSurfacePresentationModeMutex;
static std::unordered_map<Window*, SurfacePresentationMode> gSurfacePresentationModes;

static bool setSurfacePresentationMode(Window *window, SurfacePresentationMode mode) {
    std::lock_guard<std::mutex> lock(gSurfacePresentationModeMutex);
    auto &current = gSurfacePresentationModes[window];
    if (current == mode)
        return false;
    current = mode;
    return true;
...
static void forgetSurfacePresentationMode(Window *window) {
    std::lock_guard<std::mutex> lock(gSurfacePresentationModeMutex);
    gSurfacePresentationModes.erase(window);
}

// HWC isolation A/B test. When a Vulkan window starts presenting, temporarily
// remove the two extra DisplayX buffer layers from the active composition
// stack: the root buffer is cleared (the control remains as a container) and
// the CPU/X11 parent buffer is hidden/cleared. The Vulkan SurfaceControl is
// reparented directly under the DisplayX root so hiding the CPU parent does not
// hide the game. This is diagnostic only: it tells us whether Qualcomm HWC is
// rejecting the normal DisplayX layer stack because of composition-resource
// pressure / hierarchy, rather than because of the Vulkan AHardwareBuffer.
static std::mutex gHwcIsolationMutex;
static std::unordered_map<Window*, Window*> gHwcIsolationOriginalParents;

static bool markHwcIsolationActive(Window *window, Window *originalParent) {
    std::lock_guard<std::mutex> lock(gHwcIsolationMutex);
    auto [it, inserted] = gHwcIsolationOriginalParents.emplace(window, originalParent);
    return inserted;
}

static bool isHwcIsolationActive(Window *window) {
    std::lock_guard<std::mutex> lock(gHwcIsolationMutex);
    return gHwcIsolationOriginalParents.find(window) != gHwcIsolationOriginalParents.end();
}

static Window *takeHwcIsolationOriginalParent(Window *window) {
    std::lock_guard<std::mutex> lock(gHwcIsolationMutex);
    auto it = gHwcIsolationOriginalParents.find(window);
    if (it == gHwcIsolationOriginalParents.end())
        return nullptr;
    Window *parent = it->second;
    gHwcIsolationOriginalParents.erase(it);
    return parent;
}
...
void DisplayX::eventThreadLoop() {
    ...
    while (true) {
        ...
        if (currState == State::CREATE_SURFACE) {
            ...
            auto *rootWindow = windowManager ? windowManager->getRootWindow() : nullptr;
            hasSurface = rootWindow && rootWindow->control && windowTransaction;

            if (hasSurface) {
                createRootCursorControl();
                // Child controls are explicitly detached when the old Android
                // surface is destroyed. Reattach them to this new root even if
                // the platform recreates the Surface without a separate RESUME.
                restoreState = true;
            } else {
                printf("[DisplayX] CREATE_SURFACE failed — root SurfaceControl unavailable");
            }
            eventLock.notify();
        }
...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            //
            // HWC A/B TEST: configure static layer properties only when entering the
            // Vulkan path. Steady-state frames below update ONLY the buffer/fence
            // (plus geometry/visibility if they genuinely changed). This preserves the
            // large GPU-bound gain observed from marking the layer OPAQUE, while avoiding
            // redundant OPAQUE/Z mutations on every frame.
            const bool enteringVulkan =
                setSurfacePresentationMode(window, SurfacePresentationMode::Vulkan);

            if (enteringVulkan) {
                printf("[DisplayX][HWC-ISOLATION] entering Vulkan mode window=%d: OPAQUE + isolate stack",
                       window->id);

                if (pfnASurfaceTransactionSetZOrder) {
                    pfnASurfaceTransactionSetZOrder(presentTransaction, window->control, 1);
                } else {
                    printf("[DisplayX][HWC-ISOLATION] WARNING setZOrder unavailable for window %d",
                           window->id);
                }

                pfnASurfaceTransactionSetBufferTransparency(
                    presentTransaction,
                    window->control,
                    ASURFACE_TRANSACTION_TRANSPARENCY_OPAQUE);

                // Diagnostic direct-scanout stack:
                //   displayx-root (NO buffer, container only)
                //       \-- Vulkan game (OPAQUE, fullscreen)
                // CPU/X11 parent is removed from the active buffer stack.
                auto *rootWindow = windowManager ? windowManager->getRootWindow() : nullptr;
                auto *originalParent = window->parent;
                if (rootWindow && rootWindow->control && originalParent &&
                    originalParent->control && originalParent != rootWindow &&
                    pfnASurfaceTransactionReparent) {

                    if (markHwcIsolationActive(window, originalParent)) {
                        printf("[DisplayX][HWC-ISOLATION] window=%d reparent %p -> root=%p; clear root/parent buffers",
                               window->id, (void*)originalParent->control, (void*)rootWindow->control);

                        pfnASurfaceTransactionReparent(
                            presentTransaction, window->control, rootWindow->control);

                        // Keep root visible so its children remain visible, but remove
                        // its own buffer so it should not consume a hardware plane.
                        pfnASurfaceTransactionSetVisibility(
                            presentTransaction, rootWindow->control,
                            ASURFACE_TRANSACTION_VISIBILITY_SHOW);
                        pfnASurfaceTransactionSetBuffer(
                            presentTransaction, rootWindow->control, nullptr, -1);

                        // Remove the CPU parent from composition entirely.
                        pfnASurfaceTransactionSetBuffer(
                            presentTransaction, originalParent->control, nullptr, -1);
                        pfnASurfaceTransactionSetVisibility(
                            presentTransaction, originalParent->control,
                            ASURFACE_TRANSACTION_VISIBILITY_HIDE);

                        // Give HWC the simplest possible overlay candidate: full
                        // buffer -> full Android surface, no inherited X11 geometry.
                        ARect src = {
                            .left = 0,
                            .top = 0,
                        ...
                        pfnASurfaceTransactionSetGeometry(
                            presentTransaction, window->control, src, dst, 0);
                        pfnASurfaceTransactionSetVisibility(
                            presentTransaction, window->control,
                            ASURFACE_TRANSACTION_VISIBILITY_SHOW);
                    }
                } else {
                    printf("[DisplayX][HWC-ISOLATION] window=%d isolation unavailable root=%p parent=%p reparent=%p",
                           window->id, (void*)rootWindow, (void*)originalParent,
                           (void*)pfnASurfaceTransactionReparent);
                }
            }
            ...
            const bool hwcIsolationActive = isHwcIsolationActive(window);
            {
                std::lock_guard<std::mutex> cacheLock(presentGeometryCacheMutex);
                auto& geoCache = presentGeometryCache[window];
                geoChanged = !hwcIsolationActive && (!geoCache.sent
                    || geoCache.x != window->x
                    || geoCache.y != window->y
                    || geoCache.width != window->width
                    || geoCache.height != window->height);
                visChanged = !hwcIsolationActive && (!geoCache.sent || geoCache.mapped != window->mapped);
            ...
            // If this SurfaceControl was isolated for the Vulkan test and returns
            // to CPU presentation, restore the normal DisplayX hierarchy first.
            const bool enteringCpu =
                setSurfacePresentationMode(window, SurfacePresentationMode::Cpu);
            if (enteringCpu) {
                Window *originalParent = takeHwcIsolationOriginalParent(window);
                if (originalParent && originalParent->control && pfnASurfaceTransactionReparent) {
                    auto *rootWindow = windowManager ? windowManager->getRootWindow() : nullptr;
                    printf("[DisplayX][HWC-ISOLATION] restoring normal hierarchy for window=%d", window->id);
                    pfnASurfaceTransactionReparent(
                        presentTransaction, window->control, originalParent->control);
                    pfnASurfaceTransactionSetVisibility(
                        presentTransaction, originalParent->control,
                        originalParent->mapped ? ASURFACE_TRANSACTION_VISIBILITY_SHOW
                                               : ASURFACE_TRANSACTION_VISIBILITY_HIDE);
                    if (originalParent->enabled && originalParent->drawable) {
                        pfnASurfaceTransactionSetBuffer(
                            presentTransaction, originalParent->control,
                            originalParent->drawable->ahb, -1);
                    }
                    if (rootWindow && rootWindow->control && rootWindow->drawable) {
                        pfnASurfaceTransactionSetBuffer(
                            presentTransaction, rootWindow->control,
                            rootWindow->drawable->ahb, -1);
                    }
                    {
                        std::lock_guard<std::mutex> cacheLock(presentGeometryCacheMutex);
...
void DisplayX::createWindowControl(Window *window) {
    if (!window || !window->parent || !window->inputOutput) {
        printf("[DisplayX] createWindowControl: skipping window %d (parent=%p inputOutput=%d)",
               window ? window->id : -1,
               window ? (void*)window->parent : nullptr,
               window ? (int)window->inputOutput : 0);
        return;
    }

    if (!windowTransaction || !window->parent->control) {
        printf("[DisplayX] createWindowControl: skipping window %d (transaction=%p parent_control=%p)",
               window->id, (void*)windowTransaction, (void*)window->parent->control);
        return;
    }

    // ASurfaceControl_create() already returns one caller-owned reference.
    // Do NOT call ASurfaceControl_acquire() here; doing so leaks one reference
    // when destroyWindowControl() later releases only the caller-owned one.
    if (window->control) {
        printf("[DisplayX] createWindowControl: window %d already has control=%p — reusing",
               window->id, (void*)window->control);
        return;
    }

    std::string layerName = "displayx-window-" + std::to_string(window->id);
    window->control = pfnASurfaceControlCreate(window->parent->control, layerName.c_str());
    if (!window->control) {
        printf("[DisplayX] createWindowControl: FAILED window %d parent_control=%p",
               window->id, (void*)window->parent->control);
        return;
    }

    printf("[DisplayX] createWindowControl: window %d control=%p parent_control=%p name=%s",
           window->id, (void*)window->control, (void*)window->parent->control, layerName.c_str());

    pfnASurfaceTransactionSetEnableBackPressure(windowTransaction, window->control, false);
    pfnASurfaceTransactionSetVisibility(windowTransaction, window->control,
                                        ASURFACE_TRANSACTION_VISIBILITY_HIDE);
...
void DisplayX::destroyWindowControl(Window *window) {
    ...
    forgetSurfacePresentationMode(window);

    // Closing the game may destroy the Vulkan window without ever transitioning
    // back to the CPU path. Restore the background/root stack so the Activity is
    // not left blank after this diagnostic test.
    Window *isolatedOriginalParent = takeHwcIsolationOriginalParent(window);
    if (isolatedOriginalParent && windowTransaction) {
        auto *rootWindow = windowManager ? windowManager->getRootWindow() : nullptr;
        printf("[DisplayX][HWC-ISOLATION] destroy window=%d: restoring root/CPU parent", window->id);
        if (isolatedOriginalParent->control) {
            pfnASurfaceTransactionSetVisibility(
                windowTransaction, isolatedOriginalParent->control,
                isolatedOriginalParent->mapped ? ASURFACE_TRANSACTION_VISIBILITY_SHOW
                                               : ASURFACE_TRANSACTION_VISIBILITY_HIDE);
            if (isolatedOriginalParent->enabled && isolatedOriginalParent->drawable) {
                pfnASurfaceTransactionSetBuffer(
                    windowTransaction, isolatedOriginalParent->control,
                    isolatedOriginalParent->drawable->ahb, -1);
            }
        }
        if (rootWindow && rootWindow->control && rootWindow->drawable) {
            pfnASurfaceTransactionSetBuffer(
                windowTransaction, rootWindow->control, rootWindow->drawable->ahb, -1);
        }
    }
    ...
    // Explicitly remove the layer from the SurfaceFlinger tree before dropping
    // our caller-owned reference. Children remain children when a layer is
    // reparented, so detach child controls too; otherwise an orphaned subtree
    // can stay tied to the dying parent until later X11 destroy events arrive.
    if (windowTransaction && pfnASurfaceTransactionReparent) {
        for (auto *child : window->children) {
            if (child && child->control)
                pfnASurfaceTransactionReparent(windowTransaction, child->control, nullptr);
        }

        pfnASurfaceTransactionSetVisibility(windowTransaction, window->control,
                                            ASURFACE_TRANSACTION_VISIBILITY_HIDE);
        pfnASurfaceTransactionSetBuffer(windowTransaction, window->control, nullptr, -1);
        pfnASurfaceTransactionReparent(windowTransaction, window->control, nullptr);
        pfnASurfaceTransactionApply(windowTransaction);
    }

    printf("[DisplayX] destroyWindowControl: window %d releasing control=%p",
           window->id, (void*)window->control);
...
void DisplayX::mapWindow(Window *window) {
    if (!window || !window->control || !windowTransaction) {
        if (window)
            printf("[DisplayX] mapWindow: window %d has no control/transaction — skipping SHOW", window->id);
        return;
    }

    const std::string className = window->getClassName();
    if (className == windowManager->getUnviewableWMClass()) {
        printf("[DisplayX] mapWindow: window %d class='%s' matches unviewable — disabling",
               window->id, className.c_str());
...
void DisplayX::changeGeometry(Window *window, bool resized) {
    ...
    if (resized && window->drawable) {
        // Do not attach a stale buffer while the X11 client is between the
        // ConfigureWindow and repaint requests. The content path will submit
        // the new buffer once it is actually ready.
...
void DisplayX::updateCursor(Window *window) {
    if (!window || !cursorManager || !cursorManager->control || !cursorTransaction)
        return;

    auto cursor = window->cursor;
    if (!cursor || !cursor->image || !cursor->image->ahb) return;

    pfnASurfaceTransactionSetBuffer(cursorTransaction, cursorManager->control,
                                    cursor->image->ahb, -1);
    pfnASurfaceTransactionSetVisibility(
        cursorTransaction, cursorManager->control,
...
void DisplayX::updateCursorPosition() {
    if (!cursorManager || !cursorManager->control || !cursorTransaction || !env || !xServer || !cache)
        return;

    auto rootWindow = windowManager ? windowManager->getRootWindow() : nullptr;
    auto rootCursor = cursorManager->getRootCursor();
    if (!rootWindow || !rootCursor || !rootCursor->image || !rootCursor->image->ahb)
        return;
    ...
    env->DeleteLocalRef(pointWindowObj);

    auto pointWindow = windowManager->getWindow(id);
    auto cursor = (pointWindow != nullptr) ? pointWindow->cursor : nullptr;
    int x = std::clamp(cursorManager->pointer.posX, 0, rootWindow->width - 1);
    int y = std::clamp(cursorManager->pointer.posY, 0, rootWindow->height - 1);

    if (cursorVisible || (cursor && cursor->visible)) {
        if (repostCursor) {
            auto *image = (cursor && cursor->image) ? cursor->image.get() : rootCursor->image.get();
            if (image && image->ahb)
                pfnASurfaceTransactionSetBuffer(cursorTransaction, cursorManager->control, image->ahb, -1);
            repostCursor = false;
        }
        ...
        pfnASurfaceTransactionSetVisibility(cursorTransaction, cursorManager->control,
                                            ASURFACE_TRANSACTION_VISIBILITY_HIDE);
        pfnASurfaceTransactionApply(cursorTransaction);
    }
}
...
void DisplayX::createRootCursorControl() {
    if (!cursorVisible) {
        // When the app disables the cursor, do not keep an otherwise-useless
        // SurfaceControl in the HWC layer stack.
        return;
    }

    if (!cursorManager || cursorManager->control || !windowManager)
        return;

    auto rootCursor = cursorManager->getRootCursor();
    auto rootWindow = windowManager->getRootWindow();
    if (!rootCursor || !rootCursor->image || !rootCursor->image->ahb ||
        !rootWindow || !rootWindow->control)
        return;

    cursorManager->control = pfnASurfaceControlCreate(rootWindow->control, "displayx-cursor");
    if (!cursorManager->control) {
        printf("[DisplayX] createRootCursorControl: FAILED");
        return;
    }

    // ASurfaceControl_create() already gives us the owned reference.
    cursorTransaction = pfnASurfaceTransactionCreate();
    printf("[DisplayX] createRootCursorControl: control=%p", (void*)cursorManager->control);
}

void DisplayX::drawRootCursor() {
    if (!cursorVisible) {
        // Make the visibility option a real layer-stack change, not merely HIDE.
        // This keeps the cursor out of HWC validation when the user disables it.
        destroyRootCursorControl();
        return;
    }
    ...
    if (!cursorManager->control || !cursorTransaction) return;

    auto rootCursor = cursorManager->getRootCursor();
    if (!rootCursor || !rootCursor->image || !rootCursor->image->ahb) return;
...
void DisplayX::reparentWindow(Window *window, Window *parent) {
    if (!window || !parent || !window->control || !parent->control ||
        !windowTransaction || !pfnASurfaceTransactionReparent)
        return;
...
void DisplayX::createRootWindowControl() {
    if (!windowManager || !native_window) return;

    auto rootWindow = windowManager->getRootWindow();
    if (!rootWindow) return;

    if (rootWindow->control) {
        printf("[DisplayX] createRootWindowControl: replacing stale root control=%p",
               (void*)rootWindow->control);
        pfnASurfaceControlRelease(rootWindow->control);
        rootWindow->control = nullptr;
    }

    rootWindow->control = pfnASurfaceControlCreateFromWindow(this->native_window, "displayx-root");
    if (!rootWindow->control) {
        printf("[DisplayX] createRootWindowControl: FAILED");
        return;
    }

    // createFromWindow() already returns one caller-owned reference.
    ...
    printf("[DisplayX] createRootWindowControl: control=%p", (void*)rootWindow->control);
}
...
void DisplayX::destroyRootWindowControl() {
    ...
    // Detach top-level X11 controls from the old Android surface before the
    // root is released. Their caller-owned controls stay valid and can be
    // reparented to the new root by restoreControlState().
    if (windowTransaction && rootWindow->control && pfnASurfaceTransactionReparent) {
        const auto& windowTree = windowManager->getWindowTree();
        for (const auto& entry : windowTree) {
            auto *window = entry.second.get();
            if (!window || window == rootWindow || !window->control) continue;
            if (window->parent == rootWindow)
                pfnASurfaceTransactionReparent(windowTransaction, window->control, nullptr);
        }
        pfnASurfaceTransactionApply(windowTransaction);
    }
    ...
    if (rootWindow->control) {
        printf("[DisplayX] destroyRootWindowControl: releasing root control=%p",
               (void*)rootWindow->control);
        pfnASurfaceControlRelease(rootWindow->control);
        rootWindow->control = nullptr;
    }
...
void DisplayX::destroyRootCursorControl() {
    ...
    // The root cursor AHardwareBuffer belongs to CursorManager and is reused
    // across Android surface recreation / cursor visibility changes. Do not
    // release it here; the old code did so and left a live Cursor with ahb=null.
    if (cursorManager->control) {
        if (cursorTransaction && pfnASurfaceTransactionReparent) {
            pfnASurfaceTransactionSetVisibility(cursorTransaction, cursorManager->control,
                                                ASURFACE_TRANSACTION_VISIBILITY_HIDE);
            pfnASurfaceTransactionSetBuffer(cursorTransaction, cursorManager->control, nullptr, -1);
            pfnASurfaceTransactionReparent(cursorTransaction, cursorManager->control, nullptr);
            pfnASurfaceTransactionApply(cursorTransaction);
        }

        printf("[DisplayX] destroyRootCursorControl: releasing control=%p",
               (void*)cursorManager->control);
...
void DisplayX::resizeRootWindow() {
    if (!windowManager || !windowTransaction) return;
    auto rootWindow = windowManager->getRootWindow();
    if (!rootWindow || !rootWindow->control || !rootWindow->drawable) return;
...
void DisplayX::restoreControlState() {
    ...
    auto *rootWindow = windowManager->getRootWindow();
    if (!rootWindow || !rootWindow->control) return;
    ...
    for (const auto& entry : windowTree) {
        auto *window = entry.second.get();
        if (!window || window == rootWindow || !window->control ||
            !window->parent || !window->parent->control)
            continue;

        pfnASurfaceTransactionReparent(windowTransaction, window->control, window->parent->control);
        pfnASurfaceTransactionSetVisibility(
            windowTransaction, window->control,
            window->mapped ? ASURFACE_TRANSACTION_VISIBILITY_SHOW
                           : ASURFACE_TRANSACTION_VISIBILITY_HIDE);
        ...
        if (window->enabled && window->drawable)
            buffer = window->drawable->ahb;
        pfnASurfaceTransactionSetBuffer(windowTransaction, window->control, buffer, -1);
        changed = true;
    }
    ...
    repostCursor = true;
    cursorUpdate.store(cursorVisible.load(std::memory_order_relaxed), std::memory_order_relaxed);
}
