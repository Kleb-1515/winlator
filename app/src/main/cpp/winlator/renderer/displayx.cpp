...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            if (enteringVulkan) {
                printf("[DisplayX][HWC-TEST] entering Vulkan mode window=%d: applying OPAQUE + Z=1 once",
                       window->id);

                if (pfnASurfaceTransactionSetZOrder) {
                    pfnASurfaceTransactionSetZOrder(presentTransaction, window->control, 1);
                } else {
                    printf("[DisplayX][HWC-TEST] WARNING setZOrder unavailable for window %d",
                           window->id);
                }
            ...
            // CPU-drawn buffer (normal X11 path).
            //
            // HWC A/B TEST (extended): apply the same OPAQUE-once treatment used for
            // the Vulkan path. NOTE: unlike the game's single fullscreen surface,
            // these are individual Wine windows (dialogs, menus, tooltips, window
            // decorations) that may legitimately rely on per-pixel alpha (rounded
            // corners, shadows, translucent popups). Forcing OPAQUE here trades
            // that correctness for the same HWC-direct-overlay eligibility win —
            // watch for visual artifacts (hard edges, missing shadows, opaque
            // menus) after enabling this.
            const bool enteringCpu =
                setSurfacePresentationMode(window, SurfacePresentationMode::Cpu);

            if (enteringCpu) {
                printf("[DisplayX][HWC-TEST] entering CPU mode window=%d: applying OPAQUE once",
                       window->id);
