        ...
        if (!performanceHintManager) {
            ...
            if (!performanceHintSession) {
                ...
                printf("[PerfHint] CreateSession OK — tid=%d targetWorkDuration=%lld ns "
                       "(%.2f fps, perfMode=%s)",
                       tid, (long long)targetWorkDuration, targetFloat,
...
void DisplayX::setPerformanceMode(bool perfMode) {
    ...
    if (isPerformanceHintAPIAvailable() && performanceHintSession && xServer) {
        float targetFloat = perfMode ? xServer->refreshRate * 100.0f : xServer->refreshRate;
        int64_t targetWorkDuration = static_cast<int64_t>(1000000000.0f / targetFloat);
        int ret = pfnAPerformanceHintUpdateTargetWorkDuration(performanceHintSession, targetWorkDuration);
        if (ret == 0) {
            printf("[PerfHint] UpdateTargetWorkDuration OK — new target=%lld ns (%.2f fps)",
                   (long long)targetWorkDuration, targetFloat);
        } else {
