...
void DisplayX::presentThreadLoop() {
    ...
    while(true) {
        ...
        if (!window->enabled) {
            ...
            // CPU path intentionally stays behavior-identical for this A/B test.
            // We only remember the mode so a later transition back to Vulkan will
            // re-apply OPAQUE/Z once.
