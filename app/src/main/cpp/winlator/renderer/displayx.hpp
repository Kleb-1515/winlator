...
class DisplayX {
    private:
        ...
        JNIEnv *env;
        // Separate from `env` above, which belongs to the choreographer/vsync
        // thread (set in onFrameCallback64). This one is attached once at the
        // top of eventThreadLoop and must only be touched from that thread —
        // a JNIEnv* is not valid across threads.
        ...
        // Batching: window control mutations (map/unmap/geometry/create/reparent)
        // are recorded into windowTransaction under this mutex instead of being
        // applied immediately. onFrameCallback64 flushes once per vsync, so a
        // burst of window updates within one frame collapses into a single
        // SurfaceFlinger commit instead of one commit per call.
        std::mutex windowTransactionMutex;
        std::atomic_bool windowTransactionDirty{false};
        void flushWindowTransaction();
